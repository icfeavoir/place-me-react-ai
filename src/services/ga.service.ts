...
export class GAService {
  ...
  mutate(plan: Plan): Plan | null {
    ...
    const group1Seat = group1Seats[Math.floor(Math.random() * group1Seats.length)];
    ...
    const group2Seat = group2Seats[Math.floor(Math.random() * group2Seats.length)];
    const group2Member = newPlan.getGroupMemberAt(group2Seat);

    // Si les 2 sièges sont le même, on annule la mutation
    if (group1Seat.col === group2Seat.col && group1Seat.line === group2Seat.line) return null;
